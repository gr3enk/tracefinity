...
function Toggle({ checked, onChange, label, help }: { checked: boolean; onChange: (v: boolean) => void; label: string; help?: string }) {
    return (
        <div className="relative flex items-center justify-between py-2">
            ...
            <button type="button" onClick={() => onChange(!checked)} className={`relative inline-flex h-5 w-9 items-center rounded transition-colors ${checked ? "bg-accent" : "bg-elevated"}`}>
                <span
    ...
    return (
        <div className="space-y-0">
            ...
            <div className="border-t border-border mt-2 pt-1">
                <Toggle checked={config.magnets} onChange={(v) => update({ magnets: v })} label="Magnet holes" help="Holes in the base for magnets. Keeps bins locked to the baseplate." />
                {config.magnets && (
            ...
            <div className="border-t border-border mt-2 pt-1">
                ...
                {config.partial_bins && (
                    <div className="pl-3 border-l border-border-subtle ml-1 space-y-0">
                        <RadioMatrix sizeX={config.grid_x} sizeY={config.grid_y} values={config.partial_bins_values} onChange={(v) => update({ partial_bins_values: v })} />
                        <Toggle
